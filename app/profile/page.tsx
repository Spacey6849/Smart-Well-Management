            ...
            {showReset && (
              <div className="mt-2 space-y-4">
                <div className="grid gap-4 sm:grid-cols-3">
                  <div>
                    <label className={`block text-[11px] uppercase tracking-wide font-medium mb-1 ${isDark ? 'text-white/60' : 'text-gray-500'}`}>Old Password</label>
                    <input type="password" value={pwOld} onChange={e=>setPwOld(e.target.value)} className={`w-full rounded-lg border px-3 py-2 text-sm ${isDark ? 'bg-white/10 border-white/20 text-white placeholder-white/30' : 'bg-white border-gray-300 text-gray-800 placeholder-gray-400'} focus:ring-2 focus:ring-blue-400/40 outline-none`}/>
                  </div>
                  <div>
                    <label className={`block text-[11px] uppercase tracking-wide font-medium mb-1 ${isDark ? 'text-white/60' : 'text-gray-500'}`}>New Password</label>
                    <input type="password" value={pwNew} onChange={e=>setPwNew(e.target.value)} className={`w-full rounded-lg border px-3 py-2 text-sm ${isDark ? 'bg-white/10 border-white/20 text-white placeholder-white/30' : 'bg-white border-gray-300 text-gray-800 placeholder-gray-400'} focus:ring-2 focus:ring-blue-400/40 outline-none`}/>
                  </div>
                  <div>
                    <label className={`block text-[11px] uppercase tracking-wide font-medium mb-1 ${isDark ? 'text-white/60' : 'text-gray-500'}`}>Confirm Password</label>
                    <input type="password" value={pwConfirm} onChange={e=>setPwConfirm(e.target.value)} className={`w-full rounded-lg border px-3 py-2 text-sm ${isDark ? 'bg-white/10 border-white/20 text-white placeholder-white/30' : 'bg-white border-gray-300 text-gray-800 placeholder-gray-400'} focus:ring-2 focus:ring-blue-400/40 outline-none`}/>
                  </div>
                </div>
                {pwMsg && <div className={`text-xs font-medium ${/error|incorrect|short|match/i.test(pwMsg)? (isDark ? 'text-red-300' : 'text-red-600') : (isDark ? 'text-emerald-300' : 'text-emerald-600')}`}>{pwMsg}</div>}
                <div className="flex gap-3">
                  <button disabled={pwLoading} onClick={async ()=> {
                    setPwMsg(null);
                    if (!pwOld || !pwNew || !pwConfirm) { setPwMsg('All fields required'); return; }
                    ...
                    try {
                      setPwLoading(true);
                      const resp = await fetch('/api/auth/reset-password', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ oldPassword: pwOld, newPassword: pwNew, confirmPassword: pwConfirm }) });
                      const j = await resp.json().catch(()=>({}));
                      if (!resp.ok) { setPwMsg(j.error || 'Reset failed'); }
                      else { setPwMsg('Password updated'); setPwOld(''); setPwNew(''); setPwConfirm(''); setShowReset(false); }
                    } catch (e:any) { setPwMsg('Network error'); } finally { setPwLoading(false); }
                  }} className={`rounded-lg px-4 py-2 text-xs font-medium ${isDark ? 'bg-blue-600 hover:bg-blue-500 text-white disabled:opacity-50' : 'bg-blue-600 hover:bg-blue-500 text-white disabled:opacity-50'}`}>{pwLoading ? 'Saving...' : 'Save Password'}</button>
                  <button disabled={pwLoading} onClick={()=> { setShowReset(false); setPwOld(''); setPwNew(''); setPwConfirm(''); setPwMsg(null); }} className={`rounded-lg px-4 py-2 text-xs font-medium ${isDark ? 'bg-white/10 hover:bg-white/20 text-white/80' : 'bg-white border border-gray-300 text-gray-600 hover:bg-gray-50'} disabled:opacity-50`}>Cancel</button>
                </div>
